(function(){
    ...
    window.addEventListener('load', function(){
        ...
        var vsSource = [
            ...
            'void main(void) {',
                'gl_Position = mpMatrix * mwMatrix * vec4(position, 1.0);',
        ...
        uniLocations[1]  = gl.getUniformLocation(program, 'mpMatrix');
        ...
        function update(timestamp){
            ...
            // 物体を回す角度
            rotation += 0.001 * elapsedTime;
            if(1.0 < rotation) rotation -= 1.0;

            // ワールド行列の生成
            var wMatrix   = mat.identity(mat.create());
            var offset = [0.0, 0.0, -5.0];// 位置を前に動かす
            mat.translate(wMatrix, offset, wMatrix)
            var axisY = [0.0, 1.0, 0.0];// 縦軸回転でぐるぐる回す
            mat.rotate(wMatrix, 2.0*3.1415926535*rotation, axisY, wMatrix);
            var axisX = [1.0, 0.0, 0.0];// 少し傾ける
            mat.rotate(wMatrix, 2.0*3.1415926535*0.0625, axisX, wMatrix);
            ...
            gl.uniformMatrix4fv(uniLocations[1], false, pMatrix);// 射影行列の設定
